        ...
        <div class="container">
            <?php if (have_posts()) : ?>
                <?php while (have_posts()) : the_post(); ?>
                    ...
                    <?php if (is_page(['chuyen-nha-nong', 'cho-que'])) : ?>
                        ...
                        <section class="section-tight market-page-shell">
                            <?php if ($market_post && $market_post->post_type === 'aitr_market_post') : ?>
                                ...
                                <article class="glass-card market-detail-card market-detail-clean">
                                    ...
                                    <?php if ($gallery) : ?><div class="cards-4 market-gallery" style="margin-top:18px"><?php foreach ($gallery as $gallery_id) : ?><img class="media-thumb media-fit-cover" src="<?php echo esc_url(set_url_scheme((string) wp_get_attachment_image_url($gallery_id, 'large'), 'https')); ?>" alt="gallery image"><?php endforeach; ?></div><?php endif; ?>
                            ...
                            <div class="market-board-head">
                                ...
                                <div class="inline-list">
                                    <?php if (is_user_logged_in()) : ?>
                                        <a class="btn btn-primary" href="<?php echo esc_url(home_url('/portal/dashboard/#photo-library')); ?>">Đăng tin từ vườn của tôi</a>
                                    <?php endif; ?>
                    ...
                    <?php if (is_page('chuyen-nha-nong')) : ?>
                        <?php
                        $story_query = new WP_Query([
                            'post_type' => 'post',
                            'post_status' => 'publish',
                            'posts_per_page' => 20,
                            'category_name' => 'chuyen-nha-nong-blog',
                        ...
                        <section class="section-tight">
                            <div class="section-head">
                                ...
                                <h2>20 bài mới nhất trong Chuyện nhà nông</h2>
                                <p class="lead">Những bài viết mới sẽ hiện ở đây để anh/chị đọc ngay trên mobile hay desktop đều dễ theo dõi.</p>
                            </div>
                            <?php if ($story_query->have_posts()) : ?>
                                <div class="story-post-list">
                                    <?php while ($story_query->have_posts()) : $story_query->the_post(); ?>
                                        <article <?php post_class('card article-card story-post-item'); ?>>
                                            <?php if (has_post_thumbnail()) : ?>
                                                <div class="card-media">
                                                    <a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('medium_large', ['class' => 'media-thumb']); ?></a>
                                                </div>
                                            <?php endif; ?>
                                            <div class="meta-row">
                                                <span class="kicker" style="margin-bottom:0">Chuyện nhà nông</span>
                                                <span class="small subtle"><?php echo esc_html(get_the_date('d/m/Y')); ?></span>
                                            </div>
                                            <h3><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
                                            <p><?php echo esc_html(wp_trim_words(get_the_excerpt(), 28, '...')); ?></p>
                                            <a class="small-link" href="<?php the_permalink(); ?>">Đọc bài này</a>
                                        </article>
                                    <?php endwhile; ?>
                                </div>
                                <?php wp_reset_postdata(); ?>
                            <?php else : ?>
                                <div class="notice">Hiện mục Chuyện nhà nông chưa có đủ bài viết. Em sẽ bổ sung dần để chuyên mục phong phú hơn.</div>
                            <?php endif; ?>
                        </section>
                    <?php endif; ?>
                <?php endwhile; ?>
