                ...
                <?php while (have_posts()) : the_post(); ?>
                    <?php if (get_post_type() === 'aitr_market_post') : ?>
                        ...
                        <article <?php post_class('glass-card market-detail-card'); ?>>
                            ...
                            <div class="card-media media-frame media-frame-16x9" style="margin-top:12px">
                                <img class="media-thumb media-fit-cover" src="<?php echo esc_url($single_image_url ? set_url_scheme($single_image_url, 'https') : $market_fallback_url); ?>" alt="<?php the_title_attribute(); ?>" loading="lazy" onerror="this.onerror=null;this.src='<?php echo esc_url($market_fallback_url); ?>';this.alt='<?php echo esc_attr(get_the_title()); ?>';">
                            </div>
                            <?php if ($gallery) : ?>
                                <div class="cards-4 market-gallery" style="margin-top:18px">
                                    <?php foreach ($gallery as $gallery_id) : ?>
                                        ...
                                        <img class="media-thumb media-fit-cover" src="<?php echo esc_url($gallery_item_url ? set_url_scheme($gallery_item_url, 'https') : $market_fallback_url); ?>" alt="<?php the_title_attribute(); ?>" loading="lazy" onerror="this.onerror=null;this.src='<?php echo esc_url($market_fallback_url); ?>';this.alt='<?php echo esc_attr(get_the_title()); ?>';">
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                    ...
                    <?php else : ?>
                        <article <?php post_class('glass-card'); ?>>
                            <span class="eyebrow">Chuyện nhà nông</span>
                            <h1><?php the_title(); ?></h1>
