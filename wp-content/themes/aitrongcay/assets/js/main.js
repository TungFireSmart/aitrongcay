...
const sendGardenAiMessage = async (message) => {
  ...
  const body = new URLSearchParams({ action: 'aitrongcay_garden_assistant_chat', nonce: ajaxNonce, message, garden_key: activeGardenKey });
  ...
  syncGardenAiWidgets(result.data?.messages || [], result.data?.mode || 'Adapter-ready', result.data?.agentStatus || '');
};
...
const createFirstPot = async (plantName) => {
  const body = new URLSearchParams({ action: 'aitrongcay_create_first_pot', nonce: ajaxNonce, plant_name: plantName, garden_key: activeGardenKey });
  const response = await fetch(ajaxUrl, {
    method: 'POST',
    credentials: 'same-origin',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (isOnboarding) {
        ...
        document.querySelectorAll('[data-garden-ai-meta]').forEach(el => { el.textContent = 'Đang mở dashboard vườn...'; });
        document.querySelectorAll('[data-garden-ai-input]').forEach(el => { el.value = ''; });
        window.setTimeout(() => {
          window.location.href = result.redirect || `${rootUrl.replace(/\/$/, '')}/portal/dashboard/`;
        }, 900);
        return;
      }
      await sendGardenAiMessage(message);
      ...
      if (submit) {
        ...
        submit.textContent = submit.getAttribute('data-default-label') || 'Gửi';
      }
    }
  });
});
